...
export default class VodafoneBox {
  ...
  public async get<T>(endpoint: string, params?: string) {
    ...
    return await this.client.get<T>(
      `/php/${endpoint}?_n=${this.nonce}${paramsP}`,
      {
  ...
  public async login(name: string, password: string) {
    ...
    this.key = SJCLWrapper.sjclPbkdf2(
      password,
      this.salt,
      SJCLWrapper.DEFAULT_SJCL_ITERATIONS,
      SJCLWrapper.DEFAULT_SJCL_KEYSIZEBITS
    );
    const authData = "loginPassword";
    const encryptData = SJCLWrapper.sjclCCMencrypt(
      this.key,
      jsData,
      this.iv,
      authData,
      SJCLWrapper.DEFAULT_SJCL_TAGLENGTH
    );
    const loginData = {
    ...
    try {
      const { data, headers } = await this.put<any>(
        "ajaxSet_Password.php",
        loginData
      );
      ...
      if (data["p_status"].indexOf("Match") > -1) {
        ...
        this.setCookie(this.sessionId);
        this.csrfNonce = SJCLWrapper.sjclCCMdecrypt(
          this.key,
          data.encryptData,
          this.iv,
          "nonce",
  ...
  private async setSession() {
    try {
      const { data } = await this.post<{ LoginStatus: string }>(
        "ajaxSet_Session.php"
      );
      if (data.LoginStatus.indexOf("yes") == -1) {
        console.info(
          "Session could not be fully established (might still work...)"
        );
      }
    } catch (err) {
      throw new Error(
  ...
  public async getConnectedDevices() {
    try {
      ...
      const lanDevices = JSON.parse(
        data.split("json_lanAttachedDevice = ")[1].split(";")[0]
      );
      const wlanDevices = JSON.parse(
        data.split("json_primaryWlanAttachedDevice = ")[1].split(";")[0]
      );
  ...
  public async restart() {
    try {
      const { data } = await this.put<string>("ajaxSet_status_restart.php", {
